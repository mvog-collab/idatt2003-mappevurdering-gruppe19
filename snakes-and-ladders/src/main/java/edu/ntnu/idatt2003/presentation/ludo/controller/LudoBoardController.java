...
public final class LudoBoardController extends AbstractGameController<LudoBoardView> {
  ...
  private void onPieceSelected(int pieceIndex) {
    LOG.log(Level.FINE, "Piece selected by user: index {0} ", pieceIndex);
    if (!waitingForPieceSelection) {
      LOG.log(Level.FINE, "Not waiting for piece selection. Ignoring click on piece {0} ", pieceIndex);
      return;
    }
  ...
  private void processSelectedPiece() {
    LOG.log(Level.INFO, "Processing selected piece: index {0}", selectedPieceIndex);
    view.disableRollButton();
    waitingForPieceSelection = false;
    ...
    try {
      ...
      if (playerAfterMove == null) {
        LOG.log(Level.SEVERE, "Player {0} not found after move. This should not happen.", playerColorToken);
        resetAndEnableRoll();
        return;
      }
   ...
   * <p>
   * This method was generated by ChatGBT and changed to fit our structure with
   * logging. Unwanted complexity, so for further development we would have
   * constructed the path for the
   * frontend to use in the engine as well.
   * </p>
   *
