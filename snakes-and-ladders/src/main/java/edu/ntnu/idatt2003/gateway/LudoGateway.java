...
public final class LudoGateway extends AbstractGameGateway {
    ...
    public void newGame(int ignored) {
        ...
        notifyObservers(new BoardGameEvent(
            BoardGameEvent.EventType.GAME_STARTED,
            0 // Board size for Ludo
        ));
    }
    ...
    public void resetGame() {
        ...
        notifyObservers(new BoardGameEvent(
            BoardGameEvent.EventType.GAME_RESET,
            null
        ));
    }
    ...
    public void addPlayer(String name, String token, LocalDate birthday) {
        ...
        notifyObservers(new BoardGameEvent(
            BoardGameEvent.EventType.PLAYER_ADDED,
            player
        ));
    }
    ...
    public void selectPiece(int pieceIndex) {
        ...
        notifyObservers(new BoardGameEvent(
            BoardGameEvent.EventType.PIECE_SELECTED,
            selectedPieceIndex
        ));
    }
    ...
    public int rollDice() {
        ...
        notifyObservers(new BoardGameEvent(
            BoardGameEvent.EventType.DICE_ROLLED,
            lastDiceValues
        ));
    ...
    private int handlePieceMovement(Player currentPlayer, int rollValue) {
        ...
        Tile destinationTile = gameStrategy.movePiece(currentPlayer, selectedPieceIndex, rollValue, game);
        ...
        if (destinationTile != null && destinationTile != initialTile) {
            ...
            if (gameStrategy.checkWinCondition(currentPlayer, game)) {
                ...
                notifyObservers(new BoardGameEvent(
                    BoardGameEvent.EventType.WINNER_DECLARED,
                    winner
                ));
            }
        ...
        notifyObservers(new BoardGameEvent(
            BoardGameEvent.EventType.TURN_CHANGED,
            newCurrentPlayer
        ));
    ...
    public int applyPieceMovement() {
        if (game == null || game.players().isEmpty() || winner != null || selectedPieceIndex < 0) return 0;
