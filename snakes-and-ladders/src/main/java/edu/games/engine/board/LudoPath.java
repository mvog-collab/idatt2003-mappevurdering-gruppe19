...
/**
 * LudoPath – builds a colored graph of 52 + 4×6 squares
 * Handles the movement logic for a Ludo game with color-specific paths.
 */
public final class LudoPath implements MovementPath {
    ...
    public LudoPath() {
        ...
        for (LudoColor c : LudoColor.values()) {
            ...
            System.out.println("Set up " + c + " to enter goal at tile " + (entryIndex + 1) + 
                             " leading to goal tile " + lane.getFirst().id());
        }
    }

    /**
     * Get the starting tile for a specific color
     * Called when a player rolls a 6 and can leave home
     */
    ...
    public Tile next(Tile from, int steps, LudoColor owner) {
        ...
        for (int i = 0; i < steps; i++) {
            ...
            if (tile instanceof LudoRingTile && 
                tile.id() != entryPointId && // Not currently on entry point
                ...
                System.out.println("Player " + owner + " entering goal path at step " + (i+1) + " of " + steps);
            } else {
                // Regular movement
